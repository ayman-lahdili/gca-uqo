...
class CoursDiffer:
    ...
    def _compare_basic_attributes(self):
        for field in ["sigle", "titre", "cycle"]:
            ...
            if old_value != new_value:
                ...
                self.old.change["value"][field] = asdict(
                    SingleDiff(old=old_value, new=new_value)
                )
    ...
    def _compare_single_seance(self, old_seance: Seance, new_seance: Seance):
        for field in ["groupe", "campus", "ressource"]:
            ...
            if old_value != new_value:
                ...
                old_seance.change["value"][field] = asdict(
                    SingleDiff(old=old_value, new=new_value)
                )
    ...
    def _compare_activities(
        self, old_activities: List[Activite], new_activities: List[Activite]
    ):
